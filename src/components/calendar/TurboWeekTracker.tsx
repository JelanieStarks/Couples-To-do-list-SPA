...
const DayPitStop: React.FC<DayPitStopProps> = ({ date, dateStr, dayName, tasks, isToday, isPast, variant = 'classic', onExpandDay }) => {
  ...
  return (
    <div
      ref={setNodeRef}
  className={`track-day-box ${isToday ? 'today' : ''} ${isPast ? 'yesterday-fade-box' : ''} ${isOver ? 'ring-2 ring-indigo-400 ring-offset-0' : ''}`}
      ...
      {adding && (
        <div className="mb-2 p-2 rounded-md border border-slate-600 bg-slate-800/70" data-testid={`quick-add-${dateStr}`}>
          <div className="flex gap-2">
            <input
              type="text"
              className="glow-form-input flex-1"
            ...
            />
            <button type="button" className="neon-action-button" data-size="sm" data-testid={`quick-add-submit-${dateStr}`} onClick={submitQuickAdd}>Add</button>
            <button type="button" className="neon-action-button" data-size="sm" data-variant="outline" data-testid={`quick-add-cancel-${dateStr}`} onClick={cancelQuickAdd}>Cancel</button>
          </div>
        </div>
      )}
  <div className="task-lane-scroll scroll-thin flex flex-col gap-3">
        {tasks
          ...
          .map(task => (
            <TaskDriftCard key={task.id} task={task} />
          ))}
        {tasks.length === 0 && (
  ...
  className="mt-3 w-full neon-action-button"
        data-variant="soft"
        data-size="sm"
...
interface TaskDriftCardProps {
  task: any;
}

const TaskDriftCard: React.FC<TaskDriftCardProps> = ({ task }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
...
interface FullDayReplayProps {
  day: { date: Date; dateStr: string; dayName: string; tasks: Task[] };
  onClose: () => void;
}

const FullDayReplay: React.FC<FullDayReplayProps> = ({ day, onClose }) => {
